...
def Bynarium_NeuronasActualizadas():
    ...
    patron_preset = re.compile(r"preset:\s*(piramide|cuadrado|invertida)\s*,\s*([A-Za-z0-9_]+)\s*,\s*(\d+)", re.IGNORECASE) # NUEVO patrón para 'preset'
    ...
    while True:
        ...
        if match_preset:
            ...
            print(f"Preset '{topology_type}' creado exitosamente. Total de {created_neurons_count} neuronas creadas.")
            continue
        ...
        if match_help_specific:
            ...
            if cmd_help == "crea":
                ...
                print("   Ejemplo: reset")
            elif cmd_help == "preset": # NUEVA ayuda para preset
                ...
                print("\n   Tipos de Topología:")
                print("   - piramide: Capa 1: [Cantidad_Inicial] nodos (2 bits). Capa 2: [Cantidad_Inicial]/2 nodos (3 bits). Capa 3: ([Cantidad_Inicial]/2)/2 nodos (4 bits).")
                print("   - cuadrado: Capa 1: [Cantidad_Inicial] nodos (2 bits). Capa 2: [Cantidad_Inicial] nodos (3 bits). Capa 3: [Cantidad_Inicial] nodos (4 bits).")
                print("   - invertida: Capa 1: [Cantidad_Inicial] nodos (2 bits). Capa 2: [Cantidad_Inicial]*2 nodos (3 bits). Capa 3: [Cantidad_Inicial]*4 nodos (4 bits).")
                print("   Los IDs se generarán como [Prefijo_ID][Número_Capa][Índice_Nodo] (ej. 'S11', 'S23').")
                print("   Ejemplo: preset: piramide, S, 8 (Crea una pirámide con prefijo 'S', 8 nodos en la primera capa).")
            elif cmd_help == "topo":
            ...
            print("   reset")
            print("   preset: [Tipo_Topología], [Prefijo_ID], [Cantidad_Inicial]") # Actualizado en el menú principal
            print("   topo: [ID]?")
