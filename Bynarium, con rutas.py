...
def Bynarium_NeuronasActualizadas():
    ...
    patron_ruta = re.compile(r"ruta(?::\s*([A-Za-z0-9_]+)(?:\s*->\s*([A-Za-z0-9_]+))?)?(?:\s*(activa|completa))?", re.IGNORECASE) # NUEVO patrón para 'ruta'
    ...
    while True:
            ...
            try:
                ...
                if bits_int < 1:
                    ...
                    neuronas_estado[id_neurona] = {
                        ...
                        'connections_out': set() # NUEVO: Conjunto para IDs de neuronas conectadas
                    }
        ...
        if match_bif:
            ...
            if origen_id in neuronas_estado and destino1_id in neuronas_estado and destino2_id in neuronas_estado:
                ...
                if neurona_origen['current_activations_val'] >= 2: 
                    ...
                    # NUEVO: Establecer conexiones de salida
        ...
        if match_fusion:
            ...
            neurona_origen1['connections_out'].clear() # NUEVO: Limpiar conexiones de origen
            ...
            neurona_origen2['connections_out'].clear() # NUEVO: Limpiar conexiones de origen

            # NUEVO: Establecer conexiones de salida desde los orígenes al destino
            # Aunque los orígenes se reinician, la fusión establece una conexión lógica
            # Esto es si queremos que la "ruta" muestre que la fusión ocurrió.
            # Si se reinician, no tendrían conexiones de salida.
            # Por la descripción, parece que la conexión es más bien una "transformación"
            # que una conexión persistente. No añadiré conexiones aquí para evitar inconsistencias
            # con el reinicio de las neuronas origen.
            # Si la idea es que el destino "reciba" desde los orígenes, eso es implícito.
        ...
        if match_fus:
            ...
            # NUEVO: Establecer conexión de salida
        ...
        if match_preset:
            ...
            # Almacenar IDs de neuronas por capa para establecer conexiones
            ...
            for layer_idx, layer_info in enumerate(layers_config):
                ...
                for node_idx in range(1, quantity_for_layer + 1):
                    ...
                    layers_neuron_ids[layer_idx].append(id_neurona) # Almacenar ID para conexiones
            ...
            for i in range(len(layers_config) - 1):
                ...
                if topology_type == "piramide":
                    # Cada nodo en la capa superior (next_layer) recibe de 2 nodos en la capa inferior (current_layer)
                    # Esto significa que cada nodo de current_layer se conecta a un nodo en next_layer
                    # Ejemplo: S11, S12 -> S21; S13, S14 -> S22
                    # Por lo tanto, S11 se conecta a S21, S12 se conecta a S21, etc.
                    for j, source_id in enumerate(current_layer_ids):
                        target_index = j // 2 # Índice del nodo destino en la siguiente capa
                        if target_index < len(next_layer_ids):
                            ...
                            # print(f"      Conexión: {source_id} -> {target_id}") # Para depuración
                elif topology_type == "cuadrado":
                    # Cada nodo se conecta al nodo correspondiente en la siguiente capa
                    for j, source_id in enumerate(current_layer_ids):
                        if j < len(next_layer_ids): # Asegurar que el índice exista en la siguiente capa
                            target_id = next_layer_ids[j]
                            neuronas_estado[source_id]['connections_out'].add(target_id)
                            # print(f"      Conexión: {source_id} -> {target_id}") # Para depuración
                elif topology_type == "invertida":
                    # Cada nodo se conecta a 2 nodos en la siguiente capa (fan-out)
                    for j, source_id in enumerate(current_layer_ids):
                        target_index1 = j * 2
                        target_index2 = j * 2 + 1
                        if target_index1 < len(next_layer_ids):
                            ...
                            # print(f"      Conexión: {source_id} -> {target_id1}") # Para depuración
                        if target_index2 < len(next_layer_ids):
                            ...
                            # print(f"      Conexión: {source_id} -> {target_id2}") # Para depuración
            ...
            if target_id: # topo: [ID]
                ...
                if not neuronas_estado:
                    ...
                    for id_neurona, estado in neuronas_estado.items():
                        print(f"• {id_neurona}: Bits={estado['bits']}, Capacidad={estado['max_activations_capacity_str']} (val: {estado['max_activations_capacity_val']}), Activaciones={estado['current_activations_str']} (val: {estado['current_activations_val']}), Bin={estado['valor_binario_str']}, Salida={', '.join(sorted(list(estado['connections_out']))) if estado['connections_out'] else 'Ninguna'}")
            continue
        ...
        if match_ruta:
            ...
            option = match_ruta.group(3) # 'activa' o 'completa'
            ...
            if start_node_id:
                ...
                if end_node_id: # ruta: ID_Origen -> ID_Destino
                    ...
                    if paths:
                        for i, p in enumerate(paths):
                            ...
                            status = " (ACTIVA)" if is_active and option != "completa" else "" # 'completa' no chequea activa
                    ...
                    # Encontrar todas las rutas que empiezan en start_node_id
                    all_possible_paths = []
                    for potential_end_node_id in neuronas_estado.keys():
                        if potential_end_node_id != start_node_id: # No queremos rutas a sí mismo a menos que haya un ciclo explícito.
                    ...
                    if all_possible_paths:
                        for i, p in enumerate(all_possible_paths):
                            path_str = " -> ".join(p)
                            is_active = all(neuronas_estado[node]['current_activations_val'] > 0 or neuronas_estado[node]['valor_decimal'] > 0 for node in p)
                            status = " (ACTIVA)" if is_active and option != "completa" else ""
                            print(f"  Ruta {i+1}: {path_str}{status}")
                    else:
                ...
                for start_node in neuronas_estado.keys():
                    for end_node in neuronas_estado.keys():
        ...
        if match_help_specific:
            ...
            elif cmd_help == "ruta": # NUEVA ayuda para ruta
            ...
            print("   ruta: [ID_Origen] [-> [ID_Destino]] [activa|completa]?") # Actualizado en el menú principal
            print("   topo: [ID]?")
