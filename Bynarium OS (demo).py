...
def Bynarium_NeuronasActualizadas():
    ...
    while True:
        ...
        if match_crea:
            ...
            try:
                ...
                if bits_int < 1:
                    ...
                    max_activations_capacity_str = generar_sumatoria_str(formula_limit_n) # String representation
                    
                    neuronas_estado[id_neurona] = {
                        'bits': bits_int,
                        'formula_limit_n': formula_limit_n,
                        'max_activations_capacity_val': max_activations_capacity_val, # Valor numérico
                        'max_activations_capacity_str': max_activations_capacity_str, # Representación simbólica
                        'current_activations_val': 0, # Valor numérico
                        'current_activations_str': "0", # Representación simbólica
                        'valor_decimal': 0,
        ...
        if match_act:
            ...
            if id_neurona in neuronas_estado:
                ...
                if neurona['current_activations_val'] < neurona['max_activations_capacity_val']:
                    ...
                    # No hay una representación simbólica de 'act' incremental, solo se muestra el valor
                    neurona['current_activations_str'] = str(neurona['current_activations_val'])
                    print(f"Neurona '{id_neurona}' activada.")
                    print(f"   Activaciones actuales: {neurona['current_activations_str']} (Capacidad total: {neurona['max_activations_capacity_str']}).")
                else:
                    print(f"Neurona '{id_neurona}' ya alcanzó su capacidad máxima de activaciones ({neurona['max_activations_capacity_str']}).")
            else:
                print(f"Error: La neurona '{id_neurona}' no existe.")
            continue
        
        # Comandos 'inh', 'bif', 'nop' (actualizados para usar 'current_activations_val' y 'max_activations_capacity_val')
        ...
        if match_comb:
            ...
            # La neurona que cede bits debe tener al menos 3 bits para que, al perder 1,
            # sus nuevos bits (min 2) permitan calcular 'bits - 2' (min 0) para la sumatoria
            ...
            # Guardar valores antiguos para el mensaje de salida (ahora también las representaciones string)
            ...
            # ID_Llena: Sus activaciones actuales se calculan con (bits - 2) de su nuevo tamaño
            ...
            print(f"   Capacidad: {old_capacity_vacia_str} (valor: {old_capacity_vacia_str}) -> {neurona_vacia['max_activations_capacity_str']} (valor: {neurona_vacia['max_activations_capacity_val']})")
            print(f"   Activaciones: {old_activations_vacia_str} (valor: {old_activations_vacia_str}) -> {neurona_vacia['current_activations_str']} (valor: {neurona_vacia['current_activations_val']})")
            ...
            print(f"   Capacidad: {old_capacity_llena_str} (valor: {old_capacity_llena_str}) -> {neurona_llena['max_activations_capacity_str']} (valor: {neurona_llena['max_activations_capacity_val']})")
            print(f"   Activaciones: {old_activations_llena_str} (valor: {old_activations_llena_str}) -> {neurona_llena['current_activations_str']} (valor: {neurona_llena['current_activations_val']})")
