...
def Bynarium_NeuronasActualizadas():
    ...
    patron_fus = re.compile(r"fus:\s*([A-Za-z0-9_]+)\s*,\s*([A-Za-z0-9_]+)\s*,\s*(\d+)", re.IGNORECASE) # NUEVO patrón para 'fus' (transferir activaciones)
    ...
    while True:
        ...
        if match_fus:
            ...
            # Calcular cuántas activaciones puede recibir el destino
        ...
        if match_help_specific:
            ...
            print("   fus: [ID_Origen], [ID_Destino], [Cantidad]") # Actualizado en el menú principal
            print("   reset")
            print("   topo: [ID]?")
