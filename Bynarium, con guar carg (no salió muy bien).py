...
def Bynarium_NeuronasActualizadas():
    ...
    patron_guar = re.compile(r"guar:\s*([A-Za-z0-9_.-]+)", re.IGNORECASE) # NUEVO patrón para 'guar'
    patron_carg = re.compile(r"carg:\s*([A-Za-z0-9_.-]+)", re.IGNORECASE) # NUEVO patrón para 'carg'
    ...
    while True:
        ...
        if match_fusion:
            ...
            neurona_origen1['connections_out'].clear() # Limpiar conexiones de origen
            ...
            neurona_origen2['connections_out'].clear() # Limpiar conexiones de origen
        ...
        if match_ruta:
            ...
            if start_node_id:
                ...
                # Iterar sobre todas las posibles combinaciones de inicio y fin
        ...
        # Comando 'guar' (guardar estado)
        match_guar = patron_guar.match(comand)
        if match_guar:
            filename = match_guar.group(1)
            ...
            try:
                ...
                print(f"Estado de Bynarium guardado exitosamente en '{filename}'.")
            except IOError as e:
                print(f"Error al guardar el archivo '{filename}': {e}")
            continue

        # Comando 'carg' (cargar estado)
        match_carg = patron_carg.match(comand)
        if match_carg:
            filename = match_carg.group(1)
            try:
                ...
                print(f"Estado de Bynarium cargado exitosamente desde '{filename}'.")
                print("Usa 'topo:' para ver las neuronas cargadas.")
            except FileNotFoundError:
                print(f"Error: El archivo '{filename}' no se encontró.")
            except json.JSONDecodeError:
                print(f"Error: El archivo '{filename}' no es un archivo JSON válido o está corrupto.")
            except Exception as e:
                print(f"Error inesperado al cargar el archivo '{filename}': {e}")
            continue
        ...
        if match_help_specific:
            ...
            elif cmd_help == "guar": # NUEVA ayuda para guar
                print("\nguar: [nombre_archivo.json] - Guarda el estado actual de todas las neuronas (incluyendo activaciones y conexiones) en un archivo JSON.")
                print("   Ejemplo: guar: mi_red.json")
            elif cmd_help == "carg": # NUEVA ayuda para carg
                print("\ncarg: [nombre_archivo.json] - Carga el estado de las neuronas desde un archivo JSON, reemplazando el estado actual de Bynarium.")
                print("   Ejemplo: carg: mi_red.json")
            else:
            ...
            print("   guar: [nombre_archivo.json]") # Actualizado en el menú principal
            print("   carg: [nombre_archivo.json]") # Actualizado en el menú principal
            print("   topo: [ID]?")
